...
class BN_Trainer:                                        
    ...
    def train_bn(self, device, train_loader, optimizer, epoch):
        self.model.train()
    ...
    def test_bn(self, device, test_loader):
        misclassified_imgs = {}
        self.model.eval()
        test_loss = 0
        correct = 0
        ...
        with torch.no_grad():
            for data, target in test_loader:
                ...
                dnm = (l1/test_loss)*10
                lambda_l1 = 1/dnm
                test_loss = test_loss + lambda_l1*l1
                pred = output.argmax(dim=1, keepdim=True)  # get the index of the max log-probability
                correct += pred.eq(target.view_as(pred)).sum().item()
                for sample in range(data.shape[0]):
                    if (target[sample]!=pred[sample]):
                        misclassified_imgs[idx] = [data[sample].cpu(), target[sample].cpu(), pred[sample].cpu().item()]
                    idx += 1

        test_loss /= len(test_loader.dataset)
        self.test_losses.append(test_loss.cpu().item())
